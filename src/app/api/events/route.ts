...
export async function POST(req: NextRequest) {
  try {
    ...
    const { title, description, dayOfWeek, startTime, endTime, location, categoryId } = body;

    if (!title || dayOfWeek == null || !startTime || !endTime) {
      return NextResponse.json({ error: "필수 필드가 누락됐어요" }, { status: 400 });
    }

    const event = await createEvent(userId, { title, description, dayOfWeek, startTime, endTime, location, categoryId });
